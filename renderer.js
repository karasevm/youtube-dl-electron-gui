...
YouTubeDl.execute = function(args,stdoutCallback,closeCallback){
    ...
    child.on('close', function(code) {
        ...
        if (typeof closeCallback === 'function')
            closeCallback(scriptOutput,code);
    });
}
try{
    ...
    alert("Missing youtube-dl executable or cant execute it. Restart and try again.")
    console.log(e)
}
...
YouTubeDl.update = function(onUpdate,onUpdateFinish){
    YouTubeDl.execute(['-U'],data =>{
        if(data.toString().includes("Updating"))
            onUpdate();
    },() =>{onUpdateFinish()});
}
/**
...
YouTubeDl.onInit = function(){
    ...
    setTimeout(() =>{
        YouTubeDl.update(() =>{
            document.querySelector("#updateSpan").innerHTML = "Updating to new version";
        },() => {
            YouTubeDl.usable = true;
            document.querySelector("#updateSpan").innerHTML = "You are using the latest version";
 ...
 */
YouTubeDl.download = function(link,savePath,quality='(bestvideo+bestaudio/best)',callbackPercentage,callbackFinish){
    //console.log(savePath+'%(title)s.%(ext)s');
    if(YouTubeDl.usable){
        YouTubeDl.execute(['--output',savePath+'%(title)s.%(ext)s','-f',quality,link,"--ffmpeg-location",ffmpegPath],
                data =>{
                    try{
        ...
        setTimeout(() => {
            YouTubeDl.download(link,savePath,quality,callbackPercentage,callbackFinish)
        }, 10000);
    }
}
...
function updateFormats(url){
    ...
    mySelect.innerHTML = '<option selected value="(bestvideo+bestaudio/best)">Best</option>';
    YouTubeDl.getAvailableFormats(url,
        array =>{

            array.forEach(match => {
                mySelect.innerHTML = mySelect.innerHTML+'<option value="'+match[2]+'">'+match[0]+'</option>';
            });
        })
}
function downloadVideo(){
    ...
    YouTubeDl.download(link,path,quality,progress => {
